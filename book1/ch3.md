...
当函数可以作为函数的参数、返回值或者赋予给变量时，就称为Function Value。Function Value是一种特殊的数据类型，它包含了函数的地址和闭包环境。在Go语言中，函数是一等公民，可以像其他数据类型一样进行传递、赋值和使用。
...
函数指针存储的也是地址，但不是指向某种类型的数据，而是指向代码段中某个函数的第一条指令。

而Function Value是一个指针，而且是一个两级指针，为什么呢？先按住不表。

3.3.2 闭包

闭包是指一个函数值（Function Value）捕获并绑定了其引用的变量。闭包允许函数值访问其引用的变量，即使这些变量在函数值被调用时已经超出了作用域。
...
```
意思为，adder函数返回一个函数，这个函数会累加传入的参数，并返回累加结果。这里的sum变量是一个闭包，它被内部函数引用并修改。
所以当测试一下，我们运行以下代码：
```go
func main() {
    a := adder()
...
6
```
这里的sum变量是一个闭包，它被内部函数引用并修改。每次调用a函数时，sum变量都会被累加。这是因为sum变量的生命周期被延长，直到a函数不再被引用。
...
```
这里的n变量是一个闭包，它被内部函数引用。每次调用返回的函数时，都会返回n的值。这是因为n变量的生命周期被延长，直到返回的函数不再被引用。
